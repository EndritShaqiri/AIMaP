...
def read_vectorized_features(data_dir: Path | str, subset: str = "train"):
    ...
    if y_rows == nrows:
        ...
        y = np.memmap(y_path, dtype=np.int32, mode="r", shape=(nrows, n_labels))
...
def train_model(data_dir: Path | str, params: dict = {}) -> lgb.Booster:
    """
    Train LightGBM without loading the full dataset into RAM.
    Uses streaming mini-batches to build Dataset.
    """
    ...
    # LightGBM dataset that will collect batches incrementally
    train_set = None
    ...
    for start in range(0, nrows, batch_size):
        ...
        X_batch = X[batch_idx]          # memmap slice (small)
        y_batch = y[batch_idx]
        ...
        if train_set is None:
            ...
            train_set.add_features_from(X_batch)
            train_set.add_labels(y_batch)
